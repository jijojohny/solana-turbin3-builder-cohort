...
use common::*;
use introspection_amm::state::Pool;
...
fn test_burn_lp() {
    ...
    assert_eq!(
        ...
        lp_balance - burn_amount
    );
    assert_eq!(mint_supply(&svm, setup.lp_mint), lp_balance - burn_amount);
}

#[test]
fn test_burn_lp_and_withdraw_payout() {
    let mut svm = setup_svm();
    let payer = Keypair::new();
    svm.airdrop(&payer.pubkey(), LAMPORTS).unwrap();
    ...
    let vault_a_before = token_balance(&svm, setup.vault_a);
    let vault_b_before = token_balance(&svm, setup.vault_b);
    let ata_a_before = token_balance(&svm, ata(&payer.pubkey(), &setup.mint_a));
    let ata_b_before = token_balance(&svm, ata(&payer.pubkey(), &setup.mint_b));
    ...
    send_multi(&mut svm, &payer, &[&payer], &ixs);

    assert_eq!(
        token_balance(&svm, ata(&payer.pubkey(), &setup.lp_mint)),
        lp_balance - half_lp
    );
    assert!(token_balance(&svm, setup.vault_a) < vault_a_before);
    assert!(token_balance(&svm, setup.vault_b) < vault_b_before);
    assert!(token_balance(&svm, ata(&payer.pubkey(), &setup.mint_a)) > ata_a_before);
    assert!(token_balance(&svm, ata(&payer.pubkey(), &setup.mint_b)) > ata_b_before);
}
