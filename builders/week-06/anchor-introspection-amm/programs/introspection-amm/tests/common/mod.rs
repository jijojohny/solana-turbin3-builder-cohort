...
use introspection_amm::constants::{LP_MINT_SEED, POOL_SEED};
...
pub fn setup_svm() -> LiteSVM {
    ...
    svm.add_program(
        introspection_amm::ID,
        include_bytes!("../../../../target/deploy/introspection_amm.so"),
    )
    .unwrap();
    svm
}
...
pub fn lp_mint_pda(pool: &Pubkey) -> Pubkey {
    Pubkey::find_program_address(&[LP_MINT_SEED, pool.as_ref()], &introspection_amm::ID).0
}
...
pub fn initialize_ix(payer: Pubkey, mint_a: Pubkey, mint_b: Pubkey, fee_bps: u16) -> Instruction {
    ...
    Instruction::new_with_bytes(
        introspection_amm::ID,
        &introspection_amm::instruction::Initialize { fee_bps }.data(),
        introspection_amm::accounts::Initialize {
            payer,
            mint_a,
            mint_b,
            pool,
...
pub fn deposit_ix(
    ...
    Instruction::new_with_bytes(
        introspection_amm::ID,
        &introspection_amm::instruction::Deposit {
            amount_a,
            amount_b,
            min_lp_out,
        }
        .data(),
        introspection_amm::accounts::Deposit {
            depositor,
            mint_a,
            mint_b,
            pool,
...
pub fn burn_lp_ix(depositor: Pubkey, mint_a: Pubkey, mint_b: Pubkey, lp_amount: u64) -> Instruction {
    let pool = pool_pda(&mint_a, &mint_b);
    let lp_mint = lp_mint_pda(&pool);
    Instruction::new_with_bytes(
        introspection_amm::ID,
        &introspection_amm::instruction::BurnLp { lp_amount }.data(),
        introspection_amm::accounts::BurnLp {
            depositor,
            mint_a,
            mint_b,
            pool,
...
pub fn withdraw_payout_ix(
    ...
    Instruction::new_with_bytes(
        introspection_amm::ID,
        &introspection_amm::instruction::WithdrawPayout {
            min_amount_a,
            min_amount_b,
        }
        .data(),
        introspection_amm::accounts::WithdrawPayout {
            depositor,
            mint_a,
            mint_b,
            pool,
...
pub fn burn_and_withdraw_ixs(
    ...
    [
        ...
        withdraw_payout_ix(depositor, mint_a, mint_b, min_amount_a, min_amount_b),
    ]
}
...
pub fn swap_a_for_b_ix(
    ...
    Instruction::new_with_bytes(
        introspection_amm::ID,
        &introspection_amm::instruction::SwapAForB {
            amount_in,
            min_amount_out,
        }
        .data(),
        introspection_amm::accounts::Swap {
            trader,
            mint_a,
            mint_b,
            pool,
...
pub fn swap_b_for_a_ix(
    ...
    Instruction::new_with_bytes(
        introspection_amm::ID,
        &introspection_amm::instruction::SwapBForA {
            amount_in,
            min_amount_out,
        }
        .data(),
