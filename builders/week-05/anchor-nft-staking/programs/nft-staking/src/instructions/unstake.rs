...
use crate::utils::{
    decrement_staked_count, load_asset, load_collection, require_staked, unstaked_asset_attributes,
};
...
pub fn handler(ctx: Context<Unstake>) -> Result<()> {
    ...
    let (staked_at, _) = require_staked(&asset_attrs)?;
    let freeze_end = staked_at
        .checked_add(ctx.accounts.config.freeze_period as i64)
        .ok_or(StakingError::MathOverflow)?;
    require!(now >= freeze_end, StakingError::FreezePeriodNotElapsed);
    ...
    UpdatePluginV1CpiBuilder::new(&ctx.accounts.core_program.to_account_info())
        ...
        .plugin(Plugin::Attributes(unstaked_asset_attributes(&asset_attrs)?))
    ...
    UpdateCollectionPluginV1CpiBuilder::new(&ctx.accounts.core_program.to_account_info())
        ...
        .authority(Some(&ctx.accounts.authority.to_account_info()))
        .system_program(&ctx.accounts.system_program.to_account_info())
        .plugin(Plugin::Attributes(decrement_staked_count(&collection_attrs)?))
        .invoke_signed(signer)?;
